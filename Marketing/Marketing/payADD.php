...
<link rel = "icon" href = "../../Logos/title.png" 
 ...
 $ChechkPayed =  mysqli_query($con,"SELECT * FROM `market_membership` Where `add_id`= '$add_id'");
  $result = mysqli_fetch_array($ChechkPayed);
  if ($result==true){
    echo '<tr><td>NO ads to pay<td></tr>';
     }
  else {   
  $query = mysqli_query($con,"SELECT * FROM `market_ads` where `invID` = '$id' and `AddID`='$add_id'");
  while ($row = mysqli_fetch_array($query)){
    $proID = $row[2];
    $noOfshops = $row[4];
    $noOfUsers = $row[5];
    $totalSlot=$noOfshops+$noOfUsers;
...
?>
            ...
            if(isset($_POST['pay'])){
             ...
             $result = mysqli_fetch_array($ChechkPayed);
             if ($result==true){
               echo 'add already payed';
             }
             else {
              $query = "INSERT INTO `market_membership`(`add_id`,`ad_Slot`,`Price`) VALUES('$add_id','$totalSlot','$totalPrize')";
